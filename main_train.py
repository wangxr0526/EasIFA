...
from common.utils import calculate_scores_vbin_train, calculate_scores_vmulti_train, check_args, cuda, Recorder, delete_ckpt, is_valid_outputs, load_pretrain_model_state, read_model_state, save_model
...
def evaluate_vbin(
    ...
    if len(overlap_scores) != 0:
        return all_loss / len(valid_dataloader), sum(overlap_scores) / len(overlap_scores), sum(false_positive_rates) / len(false_positive_rates)
    else:
...
def evaluate_vmulti(
    ...
    with torch.no_grad():
        ...
        for batch_id, batch in enumerate(pbar):
            ...
            metrics = calculate_scores_vmulti_train(
                pred, targets, batch['protein_graph'].num_residues.tolist(), num_site_types=num_site_types)
            for key in metrics:
    ...
    if len(overlap_scores) != 0:
        return all_loss / len(valid_dataloader), sum(overlap_scores) / len(overlap_scores), sum(false_positive_rates) / len(false_positive_rates), mean_metrics
    else:
    ...
    for batch_id, batch in enumerate(pbar):
        ...
        if (report_global_step % args.evaluate_per_step == 0):
            ...
            print('\n\rEvaluation: Epoch %d/%d, batch %d/%d, global step: %d, evaluation loss %.4f, overlap score  %.4f, false positive rate  %.4f\n'
                  % (epoch + 1, args.num_epochs, batch_id + 1,
                     len(train_dataloader), report_global_step, evaluate_loss, overlap_scores, false_positive_rates))
    ...
    for batch_id, batch in enumerate(pbar):
        ...
        if (report_global_step % 10 == 0) and (comm.get_rank() == 0):
            ...
            pbar.set_description(
                ...
                % (epoch + 1, args.num_epochs, batch_id + 1,
                   len(train_dataloader), report_global_step, lr, loss.item()))
            print('\rEpoch %d/%d, batch %d/%d, global step: %d, lr: %.8f, loss %.4f'
                % (epoch + 1, args.num_epochs, batch_id + 1,
                   len(train_dataloader), report_global_step, lr, loss.item()))
            recorder.record(
                data={
        ...
        # if (report_global_step % args.evaluate_per_step == 0) and (comm.get_rank() == 0):
        if (report_global_step % args.evaluate_per_step == 0):
            evaluate_loss, overlap_scores, false_positive_rates, metrics = evaluate_vmulti(
                args,
                model=model,
                valid_set=valid_set,
                device=device,
            ...
            if comm.get_rank() == 0:
                ...
                save_model(model.state_dict(), args_dict,
                           eval_results, eval_model_save_path)
...
def main(args):
    ...
    if args.task_type == 'active-site-categorie-prediction':
        ...
        dataset = EnzymeReactionSiteTypeDataset(path=args.dataset_path,
                                            save_precessed=False, debug=debug, verbose=1, lazy=True, nb_workers=args.propcess_core)
    else:
    ...
    if args.task_type == 'active-site-position-prediction':
        ...
        model = EnzymeActiveSiteModel(
            rxn_model_path=args.pretrained_rxn_attn_model_path, 
            from_scratch=True)    # 这里传递预训练的模型只是为了初始化模型的形状，但是模型state并不继承
    else:
