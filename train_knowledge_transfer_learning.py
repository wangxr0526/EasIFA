...
def train_one_epoch(
    ...
    for batch_id, batch in enumerate(pbar):
        ...
        if report_global_step % args.evaluate_per_step == 0:
            evaluate_loss, overlap_scores, false_positive_rates, negative_accuracies = (
                evaluate(
                    args,
                    model=model,
                    valid_set=valid_set,
...
def main(args):
    ...
    for i, named_param in enumerate(model.named_parameters()):
        ...
        for pretrain_name in pretrained_model_lr_ratio:
            if pretrain_name in name:
                param_dict["lr"] = args.lr * pretrained_model_lr_ratio[pretrain_name]
    ...
    loss_fn = nn.CrossEntropyLoss(
        weight=torch.tensor([args.weight_negative, 1]).to(device)
    )

    recoder = Recorder(
        model_save_path,
        keep_num_ckpt=args.keep_num_ckpt,
        validation_recorder_columns=[
            "global step",
            "evaluation loss",
            "overlap score",
            "false positive rate",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--warmup_raio", type=float, default=0.01, help="Warmup Raio")
    parser.add_argument(
        "--weight_negative", type=float, default=0.5, help="Negative class weight"
    )
    parser.add_argument(
        "--dataset_path",
        type=str,
