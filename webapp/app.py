...
USE_FULL_SWISSPROT_CHECKPOINT = False
USE_RETRAIN_SWISSPROT_CHECKPOINT = True
...
def results_from_structure():
    ...
    if request.method == "POST":
        ...
        print(form.drawn_smiles.data, form.file.data, form.smiles.data)
    ...
    try:
        ...
        grouped_enzyme_sequence = [
            enzyme_sequence[i : i + 10] for i in range(0, len(enzyme_sequence), 10)
        ]
        grouped_enzyme_sequence_index = [
            i + 10 for i in range(0, len(enzyme_sequence), 10)
        ]
        grouped_enzyme_sequence_color = [
            enzyme_sequence_color[i : i + 10]
            for i in range(0, len(enzyme_sequence_color), 10)
        ]
        if len(enzyme_sequence) % 10 != 0:
            grouped_enzyme_sequence_index[-1] = None
            grouped_enzyme_sequence[-1] = grouped_enzyme_sequence[-1] + "".join(
                ["&" for _ in range(10 - len(grouped_enzyme_sequence[-1]))]
            )
            grouped_enzyme_sequence_color[-1] = grouped_enzyme_sequence_color[-1] + [
                "#000000" for _ in range(10 - len(grouped_enzyme_sequence_color[-1]))
            ]
    ...
    try:
        ...
        for idx, row in enumerate(query_results_df.itertuples()):
            ...
            if len(enzyme_sequence) % 10 != 0:
                ...
                grouped_enzyme_sequence_color[-1] = grouped_enzyme_sequence_color[
                    -1
                ] + [
                    "#000000"
                    for _ in range(10 - len(grouped_enzyme_sequence_color[-1]))
                ]
